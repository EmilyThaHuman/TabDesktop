...
export function ShortcutGrid() {
  ...
  const handleShortcutClick = (url: string, e: React.MouseEvent) => {
    // Don't open link if clicking on dropdown trigger
  ...
  const handleEditSubmit = async (name: string, url: string) => {
    if (editingShortcut) {
      ...
      await updateShortcut(editingShortcut.id, {
        name: name.trim(),
        url: processedUrl,
        favicon: `https://www.google.com/s2/favicons?domain=${new URL(processedUrl).hostname}&sz=16`
      })
